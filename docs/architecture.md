...
GUI ou CLI
    ...
    -> parser da demo
    -> geração de candidatos estáveis
    -> avaliações editorial e individual
    -> catálogo persistente
    -> visões por abrangência/jogador
    -> seleção final deduplicada
...
- `internal/highlights`: gera candidatos, deriva contexto objetivo, calcula as duas avaliações e ordena as visões do catálogo.
- `internal/feedback`: anexa decisões de calibração a um JSONL local, sem caminhos absolutos ou transmissão automática.
...
- Uma demo é parseada uma vez; abrangência e jogador apenas refiltram o catálogo em memória.
- O mesmo ID estável pode aparecer nas duas perspectivas, mas a seleção final e a renderização são deduplicadas.
...
As versões persistidas estão centralizadas em `internal/model/versions.go`. Metadados da demo, geração de candidatos, pontuação, diversidade editorial, master e output possuem versões independentes. Uma mudança apenas de pesos ou ordenação recalcula o catálogo sem descartar um master cuja identidade de jogador e janela de ticks permaneçam exatas; uma mudança da janela invalida esse reaproveitamento.

O manifesto guarda o catálogo completo e `selected_highlight_ids` separadamente. Na CLI, a ausência de seleção explícita usa a visão editorial equilibrada; uma seleção explicitamente vazia não renderiza nada.
...
Execute `powershell -ExecutionPolicy Bypass -File .\scripts\verify.ps1`. O script verifica formatação, módulos, `go vet`, todos os testes, os testes específicos da GUI Windows e os builds da CLI e do aplicativo. O workflow `.github/workflows/verify.yml` executa o mesmo processo no Windows.
